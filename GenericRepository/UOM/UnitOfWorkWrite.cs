...
{
	/// <summary>
	/// Represents the default implementation of the <see cref="IUnitOfWorkWrite"/> and <see cref="IUnitOfWork{TContext}"/> interface.
	/// </summary>
	...
	public class UnitOfWorkWrite<TContext> : IRepositoryFactory, IUnitOfWorkWrite where TContext : DbContext
	{
		...
		public UnitOfWorkWrite(TContext context, IHttpContextAccessor httpContextAccessor, IContextHelper contextHelper)
		{
		...
		/// <returns>A <see cref="Task{TResult}"/> that represents the asynchronous save operation. The task result contains the number of state entities written to database.</returns>
		public async Task<int> SaveChangesAsync(bool ensureAutoHistory = false, params IUnitOfWorkWrite[] unitOfWorks)
		{
			...
			using (var transaction = _context.Database.BeginTransaction())
			{
				try
				{
					...
					{
						var uow = unitOfWork as UnitOfWorkWrite<DbContext>;
