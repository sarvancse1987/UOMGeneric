...
namespace Base.DataAccess.GenericRepository.UOM
{
	/// <summary>
	/// Represents the default implementation of the <see cref="IUnitOfWorkOther"/> and <see cref="IUnitOfWork{TContext}"/> interface.
	/// </summary>
	...
	public class UnitOfWorkOther<TContext> : IRepositoryFactory, IUnitOfWorkOther where TContext : DbContext
	{
		private readonly TContext _context;
		private bool disposed = false;
		////public Microsoft.EntityFrameworkCore.Infrastructure.DatabaseFacade databaseContext { get; set; }
		private Dictionary<Type, object> repositories;
		private IHttpContextAccessor _httpContextAccessor;
		private IContextHelper _contextHelper;

		/// <summary>
		/// Initializes a new instance of the <see cref="UnitOfWorkOther{TContext}"/> class.
		/// </summary>
		/// <param name="context">The context.</param>
		public UnitOfWorkOther(TContext context, IHttpContextAccessor httpContextAccessor, IContextHelper contextHelper)
		{
			_contextHelper = contextHelper;
			_httpContextAccessor = httpContextAccessor;
			_context = context ?? throw new ArgumentNullException(nameof(context));
			////databaseContext = _context.Database;
		}
		...
		public IRepository<TEntity> GetRepository<TEntity>() where TEntity : class
		{
			...
			{
				repositories[type] = new Repository<TEntity>(_context, _httpContextAccessor, _contextHelper);
			}
		...
		/// </summary>
		/// <param name="ensureAutoHistory"><c>True</c> if save changes ensure auto record the change history.</param>
		/// <param name="unitOfWorks">An optional <see cref="IUnitOfWorkOther"/> array.</param>
		/// <returns>A <see cref="Task{TResult}"/> that represents the asynchronous save operation. The task result contains the number of state entities written to database.</returns>
		public async Task<int> SaveChangesAsync(bool ensureAutoHistory = false, params IUnitOfWorkOther[] unitOfWorks)
		{
			...
			using (var transaction = _context.Database.BeginTransaction())
			{
				try
				{
					...
					{
						var uow = unitOfWork as UnitOfWorkOther<DbContext>;
